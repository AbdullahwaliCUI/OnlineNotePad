-- =============================================
-- NotepadX Database Schema for Supabase
-- =============================================
...
-- SHARED_NOTES TABLE
-- =============================================
-- For sharing notes with specific users or publicly
CREATE TABLE public.shared_notes (
    id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
    note_id UUID REFERENCES public.notes(id) ON DELETE CASCADE NOT NULL,
    shared_by UUID REFERENCES public.profiles(id) ON DELETE CASCADE NOT NULL,
    shared_with UUID REFERENCES public.profiles(id) ON DELETE CASCADE, -- NULL for public shares
...
-- ACTIVITY_LOGS TABLE
-- =============================================
-- For tracking user activity and note interactions
CREATE TABLE public.activity_logs (
    id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
    user_id UUID REFERENCES public.profiles(id) ON DELETE CASCADE NOT NULL,
    action TEXT NOT NULL, -- 'created', 'updated', 'deleted', 'viewed', 'shared', etc.
    resource_type TEXT NOT NULL, -- 'note', 'folder', 'tag', etc.
...
CREATE INDEX idx_notes_is_public ON public.notes(is_public) WHERE is_public = TRUE;
...
CREATE INDEX idx_shared_notes_share_token ON public.shared_notes(share_token);
CREATE INDEX idx_shared_notes_is_active ON public.shared_notes(is_active) WHERE is_active = TRUE;
...
-- Note stats calculation trigger
...
CREATE VIEW public.folder_hierarchy AS
...
-- Insert some default tags for new users (you can customize this)
-- This would typically be done in your application code when a user signs up
...
-- This completes the NotepadX database schema setup
-- Next steps:
-- 1. Run this script in your Supabase SQL editor
-- 2. Configure your application's environment variables
-- 3. Test the authentication flow
-- 4. Start building your note-taking features!